...
tcpServer::tcpServer() : context_(1), socket_(new zmq::socket_t(context_, ZMQ_REP)), compression_("1"), cmp_(1), port_(5555) {
...
bool tcpServer::getMessage() {
    ...
    std::vector<char> data(str_data.begin(), str_data.end());
    cv::Mat mImg = cv::imdecode(data, 1);
